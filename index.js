    ...
    items: [
      {
        ...
        content: 'An interactive VR experience exploring non-Euclidean space. Project exhibited at the Digital Futures Lab.',
        mediaType: 'video', 
        mediaUrl: 'https://www.youtube-nocookie.com/embed/S_T1w_G8lSg' 
      }
    ]
  },
    ...
    items: [
      {
        ...
        mediaType: 'video', 
        mediaUrl: 'https://www.youtube-nocookie.com/embed/1O0E-w3sB2c'
      }
    ]
  },
];
...
const FinderWindow = ({ setActiveView, activeNav, setActiveNav, activeProjectSubfolder, setActiveProjectSubfolder }) => {
            ...
            <div className="flex-1 flex overflow-hidden">
                <div className="w-56 bg-gray-200 border-r border-gray-300 p-2 space-y-1 overflow-y-auto">
                    ...
                    {NAV_ITEMS.map(item => {
                        ...
                        return (
                            <div
                                key={item.id}
                                className={`flex items-center p-2 rounded-lg cursor-pointer transition-colors duration-150 ${activeNav === item.id ? 'bg-blue-500 text-white shadow-md' : 'hover:bg-gray-300 text-gray-700'}`}
                                onClick={() => {
                ...
                <div className="w-64 bg-white border-r border-gray-300 p-4 overflow-y-auto">
                    {activeNav === 'projects' ? (
                        <div className="space-y-4">
                            ...
                            {PROJECTS_DATA.map(folder => {
                                ...
                                return (
                                    <div
                                        key={folder.id}
                                        className={`flex items-center p-2 rounded-lg cursor-pointer transition-colors duration-150 ${activeProjectSubfolder === folder.id ? 'bg-blue-100' : 'hover:bg-gray-100'}`}
                        ...
                        <div className="p-4 pt-10">{GENERAL_CONTENT[activeNav]}</div>
                    )}
                </div>
        ...
        return (
            <div className="bg-gray-100 min-h-screen p-4 flex items-center justify-center font-sans">
                ...
                <div className="text-gray-700 p-8 bg-white rounded-lg shadow-lg">
                    <p>Window is closed. Please refresh the page to re-open.</p>
                </div>
            </div>
        );
    }
